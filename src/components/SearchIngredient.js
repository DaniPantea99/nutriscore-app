...
import { listaIngrediente } from '../db-ingrediente';

export default function SearchIngredient() {
  const [selected, setSelected] = useState(listaIngrediente[0]);
  const [query, setQuery] = useState('');

  const filteredIngrediente =
    query === ''
      ? listaIngrediente
      : listaIngrediente.filter((ingredient) =>
          ingredient.name
            .toLowerCase()
            .replace(/\s+/g, '')
        ...
        <div className="relative mt-1">
          ...
          >
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {filteredIngrediente.length === 0 && query !== '' ? (
                <div className="relative cursor-default select-none py-2 px-4 text-gray-700">
                  Nothing found.
                </div>
              ) : (
                filteredIngrediente.map((ingredient) => (
                  <Combobox.Option
                    key={ingredient.id}
                    className={({ active }) =>
